...
class _ButtomNavagationBarMakerState extends State<ButtomNavagationBarMaker> with TickerProviderStateMixin<ButtomNavagationBarMaker> {
  ...
  void initState() {
    super.initState();
    navigatorKeys = List<GlobalKey<NavigatorState>>.generate(destinations.length, (int index) => GlobalKey()).toList();
    destinationFaders = List<AnimationController>.generate(destinations.length, (int index) => buildFaderController()).toList();
